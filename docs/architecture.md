...
- This reference workspace runs on AWS. The `medallion_operational_job` cluster config (`node_type_id: m5.large`, `aws_attributes` EBS block) is the only cloud-specific content in the repo. For an Azure deployment, replace `node_type_id` with an Azure VM size (e.g. `Standard_D4ds_v5`) and remove the `aws_attributes` block — Azure VM types include managed storage by default. All other configuration (DLT pipelines, DAB structure, UC Volumes, CI/CD workflows) is cloud-agnostic. The cluster requirement itself is a constraint of CLI v0.298.0: `python_script_task` (Databricks serverless task compute) is not supported by the Terraform provider bundled with this CLI version. On a newer CLI version or with a workspace where serverless jobs are available via a supported provider, the entire cluster block is replaced by a serverless environment definition with no cloud-specific fields.
