...
- `medallion_operational_job` is a single-task job (pipeline trigger only). The intended second task — `assert_and_persist`, which runs row-count assertions and writes a summary row to `dataops_lab.ops.pipeline_run_assertions` — requires either cluster creation rights (not available in this workspace) or `python_script_task` (Databricks serverless task compute, not supported by the Terraform provider bundled with CLI v0.298.0). The script (`scripts/assert_and_persist.py`) and fixture file (`fixtures/expected_counts.json`) are in place. The task can be added once serverless task compute is supported by the bundled Terraform provider or cluster creation rights are available. Row-count validation still runs in CI via `scripts/validate_counts.py` on every PR.
